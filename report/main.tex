...
\end{lemma}
\begin{proof}
    The result is immediate after recalling the variational form of the problem as shown in \Cref{prop:variational_form} after using finite difference for approximating the derivative and replacing $u$ with $\theta u^{j} + (1- \theta) u^{j-1}$. \qedhere
    % \begin{align*}
    %     &\left( \dtu, v\right) + a(u,v) = \\
    %     &= \intS{v \dtu} + \auv = 0.
    % \end{align*}
...
\begin{prop}\label{prop:lax_milgram}
    The semi-discrete problem in \Cref{lemma:semi_discrete_prob} with $\theta = 1$ is well posed for any time step $0<\Delta t \leq \frac{1}{2 \alpha}$.
\end{prop}
\begin{proof}
...
The final experiment on the artificial solution was to perform a convergence study. For the results to be meaningful, it is important that the time discretization error does not dominate. To ensure this, the convergence study was performed using element size $h$ and time interval $\Delta t$ as shown in \Cref{tab:orders}. Performing the experiment results in \Cref{fig:conv_cos}. The plot clearly illustrates that both Crank-Nicolson and backwards Euler have a convergence order of $2$ when using P1 finite elements and order $4$ when working with P2 finite elements. \\
...
\begin{figure}[!ht]
    ...
    \caption{Convergence study for all four methods when solving \Cref{def:constructed_cos}. The $L^2$ error for all the methods was computed at the final time $T$.}
    \label{fig:conv_cos}
\end{figure}
...
\begin{figure}[!ht]
    ...
    \caption{Analytical solution (blue) versus interpolated finite element solution (dashed red) for various methods at time $t=5$ when tasked to solve \Cref{def:problem}. The PDE was solved with $h =0.5$ and $\Delta t = 0.5 $. First and second row correspond to P1 and P2 finite elements respectively. First and second column correspond to backwards Euler and Crank-Nicolson's method respectively. In the top left the maximum errors is presented for all four plots.}
...
\begin{figure}[!ht]
    ...
    \caption{Convergence study for all four methods when solving \Cref{def:problem}. The $L^2$ error for all the methods was computed at the final time $T$.}
    \label{fig:conv_true}
\end{figure}
