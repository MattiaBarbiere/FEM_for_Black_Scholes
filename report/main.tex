...
\end{proof}
\section{Question C}
\subsection{Constructed solution}
...
As a initial visual check \Cref{fig:plot_cos} illustrates the analytical versus the (interpolated) solution given by the finite element solver. All methods provide a reasonable approximation of the solution, with P2 Crank-Nicolson provided the best approximation.
...
The final experiment on the artificial solution was to perform a convergence study. For the results to be meaningful, it is important that the time discretization does not dominate. To ensure this, the convergence study was performed using element size $h$ and time interval $\Delta t$ as shown in \Cref{tab:orders}.
...
Now it is possible to isolate the convergence as a function of the element size $h$. Performing the experiment results in \Cref{fig:conv_cos}. As expected when P1 was used the convergence order is close to order $2$ while when using P2 the convergence is close order $4$. 
